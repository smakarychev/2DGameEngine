...
namespace Engine::WIP::Physics
{
    ...
    U32 PolygonPolygonContact2D::GenerateContacts(ContactInfo2D& info)
    {
        ...
        for (U32 i = 0; i < clippedPoints.size(); i++)
        {
            ...
            ContactPoint2D contactInfo{
                .LocalPoint = secondary->GetAttachedTransform().InverseTransform(clippedPoints[i]),
    ...
    U32 CircleCircleContact2D::GenerateContacts(ContactInfo2D& info)
    {
        ...
        info.Manifold->LocalReferencePoint = m_First->Center;
        manifold.LocalNormal = normal;
        manifold.ContactCount = 1;
        ContactPoint2D contactInfo{
            .LocalPoint = m_Second->Center,
    ...
    U32 EdgeCircleContact2D::GenerateContacts(ContactInfo2D& info)
    {
        ...
        ContactPoint2D contactInfo{
            .LocalPoint = m_Circle->GetAttachedTransform().InverseTransform(circleCenter - normal * distanceToPlane),
        ...
        for (auto& contactInfoEntry : *crDef.ContactList)
        {
            ...
            if (contactInfo->Manifold != nullptr)
            {
                ...
                U32 indexOfMaxDepth = manifold.Contacts[0].PenetrationDepth >
                                      manifold.Contacts[1].PenetrationDepth
                                          ? 1
                                          : 0;
    ...
    void ContactResolver::WarmStart()
    {
        for (auto& constraint : s_ContactConstraints)
        {
            ...
            for (U32 i = 0; i < manifold.ContactCount; i++)
            {
                ...
                rbA->SetAngularVelocity(
                    rbA->GetAngularVelocity() + Math::Cross2D(distA, impulseVec) * rbA->GetInverseInertia());
                rbB->SetLinearVelocity(rbB->GetLinearVelocity() - impulseVec * rbB->GetInverseMass());
                rbB->SetAngularVelocity(
                    rbB->GetAngularVelocity() - Math::Cross2D(distB, impulseVec) * rbB->GetInverseInertia());
            }
        }
    }
    ...
    void ContactResolver::ResolveTangentVelocity(const ContactConstraint2D& constraint)
    {
        ...
        for (U32 i = 0; i < manifold.ContactCount; i++)
        {
            ...
            F32 maxFriction = constraint.ContactInfo->GetFriction() * constraint.ContactInfo->AccumulatedNormalImpulses[
                i];
            F32 newImpulse = Math::Clamp(constraint.ContactInfo->AccumulatedTangentImpulses[i] + deltaImpulse,
    ...
    bool ContactResolver::ResolvePosition()
    {
        ...
        for (auto& constraint : s_ContactConstraints)
        {
            ...
            for (U32 i = 0; i < manifold.ContactCount; i++)
            {
                ...
                F32 effectiveDepth = glm::dot((contactPointWorld - refPoint), normal);
