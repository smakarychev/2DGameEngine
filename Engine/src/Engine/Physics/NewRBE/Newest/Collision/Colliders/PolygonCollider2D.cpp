...
{
    PolygonCollider2D::PolygonCollider2D(const PolygonColliderDesc2D& colDesc)
        : Collider2D(colDesc)
    {
        switch (colDesc.DataType)
        {
            case PolygonColliderDesc2D::DataType::Empty:
                break;
            case PolygonColliderDesc2D::DataType::Raw:
                BuildPolygonFromPoints(colDesc.Vertices);
                break;
            case PolygonColliderDesc2D::DataType::CookedVertices:
                m_Vertices = colDesc.Vertices;
                BuildNormals();
                m_Center = ComputeCentroid();
                break;
            case PolygonColliderDesc2D::DataType::CookedVerticesNormals:
                m_Vertices = colDesc.Vertices;
                m_Normals = colDesc.Normals;
                m_Center = ComputeCentroid();
                break;
            case PolygonColliderDesc2D::DataType::CookedFull:
                m_Vertices = colDesc.Vertices;
                m_Normals = colDesc.Normals;
                m_Center = colDesc.Center;
                break;
        }
    }
    ...
    PolygonCollider2D::Box2D PolygonCollider2D::GetAsBox() const
    {
        ...
        AABB2D bounds = AABB2D::GetFromMinMax(min, max);
        return Box2D{.Center = bounds.Center, .HalfSize = bounds.HalfSize};
    }

    AABB2D PolygonCollider2D::GenerateBounds(const Transform2D& transform) const
    {
        glm::vec2 min{std::numeric_limits<F32>::max()}, max{-std::numeric_limits<F32>::max()};
        for (auto& v : m_Vertices)
        {
        ...
        return AABB2D::GetFromMinMax(min, max);
    }

    AABB2D PolygonCollider2D::GenerateLocalBounds() const
    {
        glm::vec2 min{std::numeric_limits<F32>::max()}, max{-std::numeric_limits<F32>::max()};
        for (auto& v : m_Vertices)
        {
    ...
    MassInfo2D PolygonCollider2D::CalculateMass() const
    {
        ...
        massInfo.Inertia = inertia * m_PhysicsMaterial.Density + area * m_PhysicsMaterial.Density * (glm::dot(massInfo.CenterOfMass, massInfo.CenterOfMass) - glm::dot(centroid, centroid));
        return massInfo;
    }
    ...
    void PolygonCollider2D::BuildNormals()
    {
    ...
    glm::vec2 PolygonCollider2D::ComputeCentroid() const
    {
